...
export async function POST(req: NextRequest) {
  try {
    ...
    if (platform === 'sahibinden') {
      // Sahibinden özel durumu: N8N webhook'u ile (veya arka planda Apify ile) çekmeyi deniyoruz
      // Doğrudan browserless/fetch fail olacağı için direkt n8n'e atıyoruz.
      try {
        rawContent = await fetchViaN8n(url, platform)
      } catch (err) {
        return NextResponse.json(
          { error: 'Sahibinden ilan bilgileri çekilemedi. N8N webhook çalışmıyor olabilir.' },
          { status: 502 }
        )
      }
