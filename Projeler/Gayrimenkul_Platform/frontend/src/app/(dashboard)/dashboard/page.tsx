...
export default async function DashboardPage() {
  ...
  // 1. Aktif Müşteri (Aktif Mülk Sahipleri)
  // "ilanda olan mülk sahiplerinin sayısını yazalım" talebi doğrultusunda is_active = true ve client_type = seller (veya landlord) olanlar
  const { count: activeClientsCount } = await supabase
    .from('clients')
    .select('*', { count: 'exact', head: true })
    .eq('is_active', true)
    // Sadece satıcı veya ev sahibi portföyü temsil ettiğinden onları filtreliyoruz
    .in('client_type', ['seller', 'landlord', 'both'])
