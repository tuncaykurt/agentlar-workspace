...
function DetailModal({ listing: l, onClose, onCall }: { listing: MarketListing; onClose: () => void; onCall: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[95vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-5">
            <div className="flex-1 min-w-0">
              <h2 className="text-lg sm:text-xl font-bold text-slate-900 leading-tight">{l.title || 'İlan'}</h2>
              {(l.city || l.district) && (
            ...
            {l.price > 0 && (
              <p className="text-xl sm:text-2xl font-bold text-blue-600 flex-shrink-0">{fmtPrice(l.price, l.currency)}</p>
            )}
          </div>
            ...
            <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl mb-5 border border-slate-100">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                  ...
                  {l.seller_phone && (
                    <a href={`tel:${l.seller_phone}`} className="text-xs text-blue-600 font-medium hover:underline">
                      {l.seller_phone}
                    </a>
                  )}
                </div>
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {details.map(d => {
                  const Icon = d.icon
                  return (
                    <div key={d.label} className="flex items-center gap-2.5 p-2.5 bg-slate-50 rounded-xl border border-slate-100">
          ...
          {l.features && l.features.length > 0 && (
            <div className="mb-5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {l.features.map((f, i) => (
                  <span key={i} className="text-xs bg-blue-50 text-blue-700 px-2.5 py-1 rounded-lg border border-blue-100">{f}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {l.description && (
            <div className="mb-5">
              ...
              <div className="text-sm text-slate-600 leading-relaxed whitespace-pre-line bg-slate-50 rounded-xl p-4 border border-slate-100 max-h-48 overflow-y-auto">
                {l.description}
              </div>
            </div>
          )}
          ...
          {/* Meta */}
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 pt-4 border-t border-slate-100 mb-5">
    ...
    <div className="card mb-6 border border-slate-200">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {isDefault && <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">Varsayılan</span>}
      ...
      {open && (
        <div className="mt-4 pt-4 border-t border-slate-100">
          {loading ? (
            <div className="flex items-center justify-center py-8 text-slate-400"><Loader2 size={20} className="animate-spin mr-2" /><span className="text-sm">Yükleniyor...</span></div>
          ) : (
            <>
              ...
              <div className="flex items-center justify-between mt-3">
                <div className="flex items-center gap-2">
                  <button onClick={() => setPrompt(original)} disabled={prompt === original}
                    className="flex items-center gap-1.5 px-3 py-2 text-xs text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed">
                ...
                <button onClick={handleSave} disabled={prompt === original || saving}
                  className="flex items-center gap-1.5 px-5 py-2 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white text-sm font-medium rounded-xl">
    ...
    <div className="card mb-6 bg-gradient-to-r from-slate-800 to-slate-900 border-0">
        ...
        <div className="mb-3 p-3 bg-slate-700/50 rounded-xl border border-slate-600/50">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-slate-400 text-xs">{[selectedListing.room_count,selectedListing.m2_gross?`${selectedListing.m2_gross}m²`:null,selectedListing.district].filter(Boolean).join(' · ')}</p>
            </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <button onClick={handleTestCall} disabled={!testPhone.trim()}
          className="flex items-center justify-center gap-2 px-8 py-3 bg-green-500 hover:bg-green-600 disabled:bg-slate-600 disabled:cursor-not-allowed text-white font-semibold rounded-xl text-sm whitespace-nowrap shadow-lg shadow-green-500/20">
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Sahibinden İlanlar</h1>
          <p className="text-slate-500 text-sm mt-1">Sahibinden&apos;den çekilen {totalCount} ilan</p>
        </div>
        <button onClick={fetchListings} className="p-2.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl"><RefreshCw size={18} /></button>
      </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {['all','new','contacted','interested','converted'].map(s => (
              <button key={s} onClick={() => setFilterStatus(s)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filterStatus===s?'bg-blue-600 text-white shadow-sm':'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {listings.map(l => {
            ...
            return (
              <div key={l.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-lg transition-all duration-300 group">
                {/* Image */}
                ...
                <div className="p-4">
                  <h3 onClick={() => setSelectedListing(l)}
                    className="font-semibold text-slate-900 text-sm leading-snug mb-1 line-clamp-2 group-hover:text-blue-600 cursor-pointer">
                  ...
                  <div className="flex items-center gap-1.5 text-[11px] text-slate-500 mb-3 flex-wrap">
                    {l.room_count && <span className="flex items-center gap-1 bg-slate-50 px-2 py-0.5 rounded-md"><BedDouble size={10} /> {l.room_count}</span>}
                    {l.m2_gross > 0 && <span className="flex items-center gap-1 bg-slate-50 px-2 py-0.5 rounded-md"><Ruler size={10} /> {l.m2_gross}m²</span>}
                    {l.floor != null && l.total_floors && <span className="flex items-center gap-1 bg-slate-50 px-2 py-0.5 rounded-md"><Layers size={10} /> {l.floor}/{l.total_floors}</span>}
                    {l.age != null && <span className="flex items-center gap-1 bg-slate-50 px-2 py-0.5 rounded-md"><Calendar size={10} /> {l.age===0?'Sıfır':`${l.age}y`}</span>}
                  </div>
                  ...
                  <div className="flex items-center gap-2 py-2 border-t border-slate-100 mb-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-xs text-slate-800 font-medium truncate">{l.seller_name || 'İlan Sahibi'}</p>
                      <div className="flex items-center gap-1.5">
                        <p className="text-[10px] text-slate-400">
                          {l.seller_type === 'owner' ? 'Sahibinden' : l.seller_type === 'agency' ? 'Emlak Ofisi' : l.seller_type || 'Sahibinden'}
                        </p>
                        {l.seller_phone && (
                          <span className="text-[10px] text-blue-500 font-medium">{l.seller_phone}</span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setSelectedListing(l)}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl text-blue-700 text-xs font-medium">
