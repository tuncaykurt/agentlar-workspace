...
export default function PiyasaClient({
  listings, totalCount, statusCounts
}: {
  ...
  const filtered = listings.filter(l => {
    ...
    const matchSearch = !q || l.title?.toLowerCase().includes(q) ||
      l.city?.toLowerCase().includes(q) || l.district?.toLowerCase().includes(q) ||
      l.seller_name?.toLowerCase().includes(q)
    const matchStatus = filterStatus === 'all' || l.contact_status === filterStatus
