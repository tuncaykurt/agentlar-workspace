  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-on-surface">Portföy</h1>
          <p className="text-on-surface-variant text-sm mt-1">Tüm mülk ilanlarınız</p>
        </div>
          ...
          <div className="flex gap-2">
            ...
            <div className="flex border border-outline rounded-lg overflow-hidden">
              <button
                ...
                className={`p-2 ${viewMode === 'grid' ? 'bg-surface-container-high text-on-surface' : 'text-on-surface-variant hover:bg-surface-container-high'}`}
              >
                <Grid3x3 size={16} />
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-2 ${viewMode === 'list' ? 'bg-surface-container-high text-on-surface' : 'text-on-surface-variant hover:bg-surface-container-high'}`}
              >
                <List size={16} />
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(p => (
            <Link key={p.id} href={`/portfolio/${p.id}`}
              ...
              <div className="h-44 bg-surface-container-high relative overflow-hidden">
                {p.photos?.[0] ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <Home size={40} className="text-on-surface-variant" />
                  </div>
                )}
                {/* Durum badge */}
              ...
              <div className="p-4">
                <h3 className="font-semibold text-on-surface text-sm truncate mb-1">{p.title}</h3>
                {(p.city || p.district) && (
                ...
                <div className="flex items-center justify-between">
                  {p.price ? (
                    <p className="font-bold text-on-surface text-sm">{formatPrice(p.price, p.currency)}</p>
                  ) : (
                    <p className="text-on-surface-variant text-xs">Fiyat belirtilmemiş</p>
                  )}
          ...
          <div className="divide-y divide-outline">
            {filtered.map(p => (
              <Link key={p.id} href={`/portfolio/${p.id}`}
                ...
                <div className="w-14 h-14 bg-surface-container-high rounded-lg overflow-hidden flex-shrink-0">
                  {p.photos?.[0]
                    ? <img src={p.photos[0]} alt="" className="w-full h-full object-cover" />
                    : <div className="w-full h-full flex items-center justify-center"><Home size={20} className="text-on-surface-variant" /></div>
                  }
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-on-surface text-sm truncate">{p.title}</p>
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  {p.price && <p className="font-semibold text-on-surface text-sm">{formatPrice(p.price, p.currency)}</p>}
                  <span className={`text-xs px-2 py-0.5 rounded-full ${statusColors[p.status]}`}>{statusLabels[p.status]}</span>
                  <ChevronRight size={16} className="text-on-surface-variant group-hover:text-on-surface-variant" />
                </div>
              </Link>
            ))}
          </div>
