...
class GridBacktestEngine:
    ...
    def run(self, ohlcv: list) -> dict:
        ...
        for i in range(self.bb_period, len(df)):
            ...
            # Tüm açık pozisyonların toplam notional ve unrealized PnL hesapla
            ...
            # Cross margin likidasyon: bakiye + unrealized <= maintenance margin
            ...
            if state["filled"] and (balance + total_unrealized) <= maintenance_margin:
                ...
                for lvl in list(state["filled"]):
                    ...
                    pos_val = lvl_qty * ep
                    fee = lvl_qty * price * self.taker_fee_pct  # Sadece çıkış taker fee
                    trades.append({
