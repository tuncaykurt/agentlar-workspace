...
async def _get_zero_fee_symbols(exchange_client) -> list[dict]:
    ...
    symbols = []
    # Sadece API ile trade edilemeyen contractları filtrele (apiAllowed=false)
    for symbol, market in exchange_client.markets.items():
        ...
        # MEXC info'dan apiAllowed kontrol et — false ise API ile trade edilemez
        info = market.get("info", {})
        if isinstance(info, dict):
            api_allowed = info.get("apiAllowed")
            if api_allowed is not None and str(api_allowed).lower() == "false":
                continue
